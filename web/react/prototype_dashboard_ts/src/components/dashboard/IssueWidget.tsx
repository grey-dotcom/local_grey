 ...
 *   allIssues 전체(ALL 필터 기준) 카드 수를 전달.
 *
 * [BE 연동 가이드]
...
const FILTER_TABS: { key: IssueFilter; label: string }[] = [
  { key: 'ALL',       label: '전체' },
  { key: 'ISSUE',     label: '이슈' },
  { key: 'HOLD',      label: '보류' }, // ⚠️ 68차: ON_HOLD → HOLD
...
function filterIssues(issues: IssueReport[], filter: IssueFilter): IssueReport[] {
  switch (filter) {
    ...
    case 'ISSUE':     return issues.filter(i => i.issueStatus === 'REPORTED'); // ⚠️ 68차: 접수됨만
    case 'HOLD':      return issues.filter(i => i.issueStatus === 'HOLD');     // ⚠️ 68차: ON_HOLD → HOLD
    case 'COMPLETED': return issues.filter(i => i.issueStatus === 'RESOLVED'); // ⚠️ 68차: COMPLETED → RESOLVED
...
export function IssueWidget({ isMobile = false, isSingleColumn = false, desktopCols = 2, style, onCountChange }: IssueWidgetProps) {
  ...
  useEffect(() => {
    onCountChange?.(allIssues.length);
  }, [allIssues.length, onCountChange]);
    ...
    <div style={{ background: 'white', borderRadius: 8, outline: '1px #EEEEEE solid', outlineOffset: -1, display: 'flex', flexDirection: 'column', overflow: 'hidden', ...style }}>
      ...
      <div ref={listRef} style={{ paddingTop: LIST_PADDING_V, paddingLeft: isMobile ? 16 : 24, paddingRight: isMobile ? 16 : 24, paddingBottom: LIST_PADDING_V, display: 'flex', flexDirection: 'column', gap: CARD_GAP, ...(needsScroll ? { overflowY: 'auto', maxHeight: cardListMaxH ?? 'none', opacity: measured ? 1 : 0, transition: 'opacity 0.15s ease' } : { overflowY: 'visible' }) }}>
        {filtered.length === 0 ? (
          ...
          filtered.map(issue => (
            <IssueCard key={issue.issueId} issue={issue} isMobile={isMobile} onStatusChange={handleStatusChange} />
          ))
        )}
      </div>
    </div>
